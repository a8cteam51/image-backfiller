...
namespace WPCOMSpecialProjects\1;
...
class Plugin {
	...
	public function is_active( string &$minimum_wc_version = null ): bool {
		...
		if ( null === $minimum_wc_version ) {
			$updated_plugin_metadata = \get_plugin_data( \trailingslashit( WP_PLUGIN_DIR ) . WPCOMSP_51_BACKFILL_BASENAME, false, false );
		...
		return true === WPCOMSP_51_BACKFILL_REQUIREMENTS;
	}
				...
				static function() use ( $minimum_wc_version ) {
					if ( \is_null( $minimum_wc_version ) ) {
						$message = \wp_sprintf(
							...
							__( '<strong>%1$s (v%2$s)</strong> requires WooCommerce. Please install and/or activate WooCommerce!', 'wpcomsp-image-backfiller' ),
							WPCOMSP_51_BACKFILL_METADATA['Name'],
							WPCOMSP_51_BACKFILL_METADATA['Version']
						);
					} else {
						$message = \wp_sprintf(
							/* translators: 1. Plugin name, 2. Plugin version, 3. Minimum WC version. */
							__( '<strong>%1$s (v%2$s)</strong> requires WooCommerce %3$s or newer. Please install, update, and/or activate WooCommerce!', 'wpcomsp-image-backfiller' ),
							WPCOMSP_51_BACKFILL_METADATA['Name'],
							WPCOMSP_51_BACKFILL_METADATA['Version'],
							$minimum_wc_version
						);
