...
The Image Backfiller plugin adds a new WP-CLI command: `wp backfill` with a single get subcommand:

`wp backfill get --domain= [--tags=] [--protocol=] [--num-posts=] [--posts=] [--import-duplicates] [--include-params] [--dry-run] [--verbose]`

The command will locate any images at the indicated domain, download them to the site, and update image links to point to the downloaded image. Options allow for shifts in that behavior.
